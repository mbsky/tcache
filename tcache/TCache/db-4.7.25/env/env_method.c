...
#define	OK_FLAGS							\
	(DB_AUTO_COMMIT | DB_CDB_ALLDB | DB_DIRECT_DB |			\
	    ...
	    DB_TXN_SNAPSHOT |	DB_TXN_WRITE_NOSYNC | DB_YIELDCPU | DB_DISABLE_LRU /*[Igor]: Make Disable LRU flag to be valid*/)
	...
	//[Igor]: Hack: Just set the private variable and clear the flag
	if(FLD_ISSET(flags, DB_DISABLE_LRU))
	{ 
		dbenv->disableLRU=1;
		FLD_CLR(flags, DB_DISABLE_LRU);
	}
